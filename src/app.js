(function () {
  ...
  function createSyntheticCity(cityKey, baseLat, baseLng, count) {
    ...
    for (let i = 1; i <= count; i += 1) {
      ...
      const jitterStep = Math.floor((i - 1) / stream.anchors.length) * 0.003;
      list.push({
        id: `${cityKey}-${i}`,
        name: `${anchor.name} • камера ${i}`,
        district: city,
        location: `${city}, ${anchor.name}`,
        lat: anchor.lat + jitterStep,
        lng: anchor.lng - jitterStep,
        ...
        accessNote: "Источник взят напрямую из каталога EarthCam / SkylineWebcams / WorldCam. Если встраивание недоступно, откройте оригинал."
      });
    }
    return list;
  }
