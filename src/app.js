(function () {
  ...
  function createSyntheticCity(cityKey, baseLat, baseLng, count) {
    ...
    for (let i = 1; i <= count; i += 1) {
      ...
      list.push({
        ...
        embedUrl: stream.sourceUrls[(i - 1) % stream.sourceUrls.length],
  ...
  function renderCountryAreas() {
    ...
    countries.forEach((countryKey) => {
      ...
      if (!countryMeta || !countryMeta.bounds) {
        return;
      }

      const isActive = state.country === countryKey;
      const rect = L.rectangle(countryMeta.bounds, {
      ...
      rect.bindTooltip(countryMeta.label, {
        permanent: true,
        direction: "center",
